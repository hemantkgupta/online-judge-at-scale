...
Consumes `evaluated_results`, caches each verdict in Redis (`verdict:{submissionId}` STRING with 24 h TTL), and pushes via STOMP-over-SockJS to subscribed WebSocket sessions. The WebSocket endpoint is `/ws`; clients SUBSCRIBE to `/topic/leaderboard/{contestId}` for contest-wide updates and `/topic/verdicts/{userId}` for per-user verdict feeds. Reads from a **sharded** sorted-set scheme `leaderboard:{contestId}:shard:{idx}` routed by `ScoreRangeShardRouter`. **Stand-in writer (2026-05-19):** on each ACCEPTED `system`/`final` verdict, the same `@KafkaListener` ZADDs the user into the score-range ZSETs and PUBLISHes on `score_updates:{contestId}` — penalty = 0, byte-identical Lua to scoring-pipeline's `RedisLeaderboardSink`. Gated by `app.leaderboard.writer.enabled` (default `true`); flips OFF when scoring-pipeline (PR #8) takes over. See [`services/leaderboard-service.md`](./services/leaderboard-service.md) §3.6 for the cutover procedure. Spring Boot 3.2.4, port 8082; Dockerfile + compose entry shipped, not yet deployed.
...
Flink DataStream job that would consume `evaluated_results`, apply per-contest scoring rules (first-AC-wins / time-penalty / partial-credit), and write the resulting score deltas to the Redis sorted-set leaderboard. Treats the verdict stream as source of truth and the Redis state as a materialised view. **Status (2026-05-19):** job code and tests ready; productionising (Flink JM/TM in compose, fat-JAR submission, cross-region MM2 smoke harness) tracked in PR #8. `build.gradle` declares Flink as `compileOnly`; `main()` expects an external JobManager + TaskManager. Until PR #8 lands, the live writer is leaderboard-service's stand-in `LeaderboardWriter` (§4.6) — ZADD-by-points with penalty = 0; ICPC penalty math arrives with Flink. The cutover is a flag flip on leaderboard-service after rate-parity is observed.
...
|---|---|---|---|
| `leaderboard:{contestId}:shard:{idx}` | ZSET | leaderboard-service stand-in writer (default) / scoring-pipeline after cutover; leaderboard-service reads | Per-contest ranking, score-range-sharded via `ScoreRangeShardRouter` (common module). `{idx}` is the shard ordinal. Reads aggregate across shards. **Populated today** via the stand-in writer (`app.leaderboard.writer.enabled=true`, default). Flips to Flink-owned after the PR #8 cutover (§4.6 / §4.7). |
...
| scoring-pipeline / Flink productionisation | §4.7 + PR #8 (JM/TM + smoke harness + MM2). M1 closed via the leaderboard-service stand-in writer (§4.6, landed 2026-05-19); cutover = flip `app.leaderboard.writer.enabled=false` once PR #8 lands and rate parity is observed. | Low — feature-complete on the stand-in; cutover is an operator flag flip |
